...
public function string of_encrypt (string senha)
end prototypes
...
public function boolean of_validar ();String ls_cpf, ls_senha, ls_senhae, ls_nome, ls_funcao, ls_nova_senha, ls_senha_encrypt
...
ls_senha_encrypt = of_encrypt( ls_senha ) 

If ls_senha_encrypt = '' Then
	Msg('Erro ao criar Usuario')
	Return False
End If

dw_cad_funcionario.SetItem(1,'fun_senha', ls_senha_encrypt) 
...
public function string of_encrypt (string senha);nv_Encrypt ln_Encrypt

ln_Encrypt = Create nv_Encrypt 

return ln_Encrypt.of_cryptpw( senha )
